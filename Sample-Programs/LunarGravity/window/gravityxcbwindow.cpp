...
GravityXcbWindow::GravityXcbWindow(const char *win_name, const uint32_t width, const uint32_t height, bool fullscreen) :
    ...
    if (xcb_connection_has_error(m_connection) > 0) {
        logger.LogError("GravityXcbWindow::GravityXcbWindow Xcb Connection failed");
        exit(-1);
    }
...
GravityXcbWindow::~GravityXcbWindow() {
    m_end_thread = true;
    CloseGfxWindow();
}
...
static void handle_xcb_event(GravityXcbWindow *window, const xcb_generic_event_t *event) {
...
static void xcb_window_thread(GravityXcbWindow *window) {
    GravityLogger &logger = GravityLogger::getInstance();
...
bool GravityXcbWindow::CreateGfxWindow(VkInstance &instance) {
    ...
    if (VK_SUCCESS != vk_result) {
        std::string error_msg = "GravityXcbWindow::CreateGfxWindow - vkCreateXcbSurfaceKHR failed "
                                "with error ";
        error_msg += vk_result;
...
bool GravityXcbWindow::CloseGfxWindow() {
    if (NULL != m_window_thread) {
