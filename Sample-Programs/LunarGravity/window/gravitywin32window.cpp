        ...
        case WM_KEYUP: {
            ...
            if (add_key) {
                if (event_list.SpaceAvailable()) {
                    ...
                    logger.LogError(
                        "GravityWin32Window::WindowCallbackProc No space in event "
...
static void window_thread(GravityWin32Window *window, const char *m_win_name, HINSTANCE hinst, HWND *hwnd, POINT *point,
    ...
    if (!RegisterClassEx(&win_class_ex)) {
        logger.LogError(
            "GravityWin32Window::CreateGfxWindow - Failed attempting"
    ...
    if (window->IsFullscreen()) {
        ...
        if (DISP_CHANGE_SUCCESSFUL != ChangeDisplaySettings(&dev_mode_settings, CDS_FULLSCREEN)) {
            if (IDYES == MessageBox(nullptr, "Failed fullscreen at provided resolution.  Go Windowed?", m_win_name,
                ...
                // Give up
                logger.LogError(
                    "GravityWin32Window::CreateGfxWindow - Failed to create"
    ...
    *hwnd = CreateWindowEx(ext_win_style, m_win_name, m_win_name, win_style, 100, 100, wr.right - wr.left, wr.bottom - wr.top,
    ...
    if (VK_SUCCESS != vk_result) {
        std::string error_msg =
            "GravityWin32Window::CreateGfxWindow - vkCreateWin32SurfaceKHR failed "
            "with error ";
        error_msg += vk_result;
...
void GravityWin32Window::TriggerQuit() { PostQuitMessage(0); }
