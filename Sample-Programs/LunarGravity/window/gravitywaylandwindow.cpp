/*
 * LunarGravity - gravitywaylandwindow.cpp
 *
...
static void RegistryHandleGlobal(void *data, struct wl_registry *registry,
    ...
    GravityWaylandWindow *wayland_window = data;
    GravityLogger &logger = GravityLogger::getInstance();

    if (strcmp(interface, "wl_compositor") == 0) {
        if (!BindRegistryCompositorInterface(registry, name, interface)) {
            logger.LogError("GravityWaylandWindow::RegistryHandleGlobal failed to register "
...
GravityWaylandWindow::GravityWaylandWindow(const char *win_name, const uint32_t width, const uint32_t height,
                                 bool fullscreen) :
    GravityWindow(win_name, width, height, fullscreen) {
    GravityLogger &logger = GravityLogger::getInstance();
    m_display = wl_display_connect(nullptr);
    if (m_display == nullptr) {
        logger.LogError("GravityWaylandWindow::GravityWaylandWindow Wayland Display Connection failed");
        exit(-1);
    }
...
GravityWaylandWindow::~GravityWaylandWindow() {
    CloseGfxWindow();
}

bool GravityWaylandWindow::BindRegistryCompositorInterface(struct wl_registry *registry, uint32_t name,
                                     const char *interface) {
    m_compositor = wl_registry_bind(registry, name, &wl_compositor_interface, 3);
    return true;
}
bool GravityWaylandWindow::BindRegistryShellInterface(struct wl_registry *registry, uint32_t name,
                                const char *interface) {
...
bool GravityWaylandWindow::CreateGfxWindow(VkInstance &instance) {
    GravityLogger &logger = GravityLogger::getInstance();

    m_window = wl_compositor_create_surface(m_compositor);
    if (!m_window) {
        logger.LogError("GravityWaylandWindow::CreateGfxWindow - Can not create wayland_surface "
                        "from compositor");
        return false;
    }
    ...
    if (!m_shell_surface) {
        logger.LogError("GravityWaylandWindow::CreateGfxWindow - Can not create shell_surface "
    ...
    if (VK_SUCCESS != vk_result) {
        std::string error_msg = "GravityWaylandWindow::CreateGfxWindow - vkCreateWaylandSurfaceKHR failed "
                                "with error ";
        error_msg += vk_result;
...
bool GravityWaylandWindow::CloseGfxWindow() {
